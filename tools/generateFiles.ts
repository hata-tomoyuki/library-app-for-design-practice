...
import { generateRequestDto, generateResponseDto, generateUseCase, generateUseCaseInterface } from "./templates/useCaseLayer";
...
async function generateUseCaseLayer() {
    ...
    writeFile(
        path.join(basePath, 'usecases', `${entityName}`, `${lowercaseFirst(useCaseName)}UseCaseInterface.ts`),
        useCaseInterfaceContent
    );

    const useCaseContent = generateUseCase(entityName, useCaseName);
    writeFile(
        path.join(basePath, 'usecases', `${entityName}`, `${lowercaseFirst(useCaseName)}UseCase.ts`),
        useCaseContent
    );

    const requestDtoContent = generateRequestDto(useCaseName);
    writeFile(
        path.join(basePath, 'dtos', `${entityName}`, `${lowercaseFirst(useCaseName)}RequestDto.ts`),
        requestDtoContent
    );

    const responseDtoContent = generateResponseDto(useCaseName);
    writeFile(
        path.join(basePath, 'dtos', `${entityName}`, `${lowercaseFirst(useCaseName)}ResponseDto.ts`),
        responseDtoContent
    );
}
